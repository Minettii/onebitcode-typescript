...
function addCrew(
	crewMember: string,
	spaceship: { crew: Array<string>; crewLimit: number }
) {
	if (spaceship.crewLimit === spaceship.crew.length) {
		alert(
			"Não foi possível adicionar um membro à tripulação, o limite de tamanho máximo da tripulação foi atingido"
		);
	} else {
...
function sendToMission(spaceship: {
	...
	if (spaceship.inMission) {
		alert(
			"Não foi possível enviar a nave em uma missão, pois a nave já está em uma missão."
		);
	} else if (spaceship.crew.length < Math.floor(spaceship.crewLimit / 3)) {
		alert(
			"Não foi possível enviar a nave em uma missão, pois menos de um terço de sua tripulação está preenchida."
		);
	} else {
